...
import { hasFunctionReturnValue } from "./hasFunctionReturnValue";
import { isIdentifierStart, isIdentifierPart } from "./Tokenizer";
import { getDTName } from "./names";
import { ScriptTarget } from "./ScriptTarget";
import { usesFunctionArguments } from "./usesFunctionArguments";
...
function isLegalIdentifier(s: string) {
	...
	if (!isIdentifierStart(s.charCodeAt(0), ScriptTarget.Latest)) {
		return false;
	}
	for (let i = 1; i < s.length; i++) {
		if (!isIdentifierPart(s.charCodeAt(i), ScriptTarget.Latest)) {
			return false;
		}
	}
...
function getTopLevelDeclarations(name: string, obj: any): dom.NamespaceMember[] {
	...
	function getResult(): dom.NamespaceMember[] {
		if (typeof obj === 'function') {
			const funcType = getParameterListAndReturnType(obj);
			...
			if (isClasslike(obj)) {
				...
				const info = getParameterListAndReturnType(obj);
...
function getParameterListAndReturnType(obj: Function): [dom.Parameter[], dom.Type] {
	if (isNativeFunction(obj)) {
		...
		const parameters = getFunctionParameters(obj)

		if (parameters) {
			params = parameters.map(p => create.parameter(`${p}`, dom.type.any));
			if (usesFunctionArguments(obj)) {
				params.push(create.parameter('args', dom.type.array(dom.type.any), dom.ParameterFlags.Rest));
			}
		}
		return [params, hasFunctionReturnValue(obj) ? dom.type.any : dom.type.void];
	}
}
